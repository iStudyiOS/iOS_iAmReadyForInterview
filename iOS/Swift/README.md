...
<details>
  <summary> Strong, Weak, Unowned의 차이 </summary>
  </br>
...
```
  ...
  - `Strong` 타입 변수는 인스턴스 생성 시 RC를 증가시킨다.
    - `Strong` 으로 인해 발생할 수 있는 `강한 참조 순환`을 해결하기 위해 `Weak`을 사용한다.

## Weak
- `Strong`과 달리 RC 증가 시키지 않는다.
- ARC에서 메모리가 해제되면 같이 사라지게 된다.
  - 위의 특성으로 인해 자동으로 `nil`값을 가지게 되므로 `Optional` 타입이라는 것을 유추할 수 있다.
